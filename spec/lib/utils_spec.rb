  ...
  describe '.resolve_url' do 
    context 'arguments' do
      it 'should accept Strings as arguments' do
        ...
        expect( Utils.resolve_url href, ab).to eq 'http://hello.com/world'
      end
      ...
      describe 'error conditions' do
        context 'without exactly 2 arguments' do
          it 'should raise ArgumentError' do 
            expect{Utils.resolve_url 'http://example.com' }.to raise_error ArgumentError
          end
        end

        context 'either argument is nil' do
          it 'should raise Argument Error' do
            expect{Utils.resolve_url  nil, 'http://example.com'}.to raise_error ArgumentError
            expect{Utils.resolve_url 'http://example.com', nil}.to raise_error ArgumentError
          end
        end

        context 'arguments are neither Strings nor URIs' do
          it 'should raise ArgumentError if arguments are not a String nor URI' do 
            expect{Utils.resolve_url 42, 'http://example.com' }.to raise_error ArgumentError
          end
        end

        context 'second argument is not absolute URI' do
          it 'should raise ArgumentError' do
            expect{Utils.resolve_url 'http://example.com', 'world.html' }.to raise_error ArgumentError
          end
        end
      end
    end
    ...
    describe 'absolute path resolution' do 
      context 'when first argument is absolute' do 
        it 'should be idempotent' do 
          expect(Utils.resolve_url( 'http://propublican.org/', 'https://example.com')).to eq 'http://propublican.org/'
        end
      ...
      context 'when first arg is relative' do
        context 'when 2nd arg has path with no trailing slash' do
          ...
          it 'should return #hash page anchor ' do 
            expect(Utils.resolve_url( '#bang', @page_url)).to eq 'http://www.propublica.org/index.html#bang'
          end

          it 'should return query params' do           
            expect(Utils.resolve_url( '?q=1', @page_url)).to eq 'http://www.propublica.org/index.html?q=1'
          end
          ...
          it 'should append a relative path to host/..' do 
            expect(Utils.resolve_url( 'dir.html', @page_url)).to eq 'http://www.propublica.org/dir.html'
          end
        end
        ...
        context 'when 2nd arg has path w/ trailing slash, i.e. an actual directory' do 
          ...
          it 'should append a subdir path to existing path' do 
            expect(Utils.resolve_url( 'dir', @page_url)).to eq 'http://www.propublica.org/pages/dir'
          end
    ...
    describe 'normalization' do
      it 'normalizes host names' do
        expect(Utils.resolve_url 'world.html', 'http://WWW.HeLLO.coM/').to eq 'http://www.hello.com/world.html'
      end

      it 'escapes improper URI strings' do
        expect(Utils.resolve_url 'you and me', 'http://hello.com').to eq 'http://hello.com/you%20and%20me'
      end
    end
